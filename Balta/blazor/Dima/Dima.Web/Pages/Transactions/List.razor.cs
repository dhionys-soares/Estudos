    ...
    public partial class ListTransactionsPage : ComponentBase
    {
        ...
        public Func<Transaction, bool> Filter
            => transaction =>
            {
                ...
                return transaction.Id.ToString().Contains(SearchTerm, StringComparison.OrdinalIgnoreCase) || transaction.Title.ToString().Contains(SearchTerm, StringComparison.OrdinalIgnoreCase);
            };

        public async void OnDeleteButtonClickedAsync(long id, string title)
        {
            var result = await DialogService.ShowMessageBox("Atenção!", $"Ao prosseguir, o lançamento {title} será excluído. Esta ação é irreversível! Deseja continuar?", yesText: "Excluir", cancelText: "Cancel");
            if (result is true)
